...
CACHE="/tmp/slotserver-new-stats-`echo $SLOT`.cache"
...
if [ "$METRIC" = "connections" ]; then
    /bin/cat $CACHE | /bin/grep "Connections:" | /usr/bin/awk '{print $4}'
elif [ "$METRIC" = "queue" ]; then
    /bin/cat $CACHE | /bin/grep "Queue:" | /usr/bin/awk '{print $4}'
else
  exit 0
fi
